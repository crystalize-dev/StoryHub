    ...
    return (
        <div className="flex h-full w-full flex-col items-center justify-center gap-8 bg-gradient-to-br from-light-bg to-primary-hover p-8 dark:from-dark-bg dark:to-primary">
            ...
            <motion.form
                layoutId="loginForm"
                onSubmit={(e) => submit(e)}
                className={`relative flex max-h-[95%] max-w-[95%] flex-col items-center gap-4 rounded-2xl bg-white !bg-opacity-50 p-6 shadow-md dark:bg-black`}
            >
                <div
                    className="group absolute -left-14 -top-14 flex cursor-pointer items-center gap-1 rounded-full px-4 py-2 text-sm hover:bg-white/20 dark:hover:!bg-black/20"
                    onClick={() => router.push('/login')}
                >
                    <Icon icon="arrowLeft" className="h-6 w-6 scale-75" />
                    <p className="relative -left-2 whitespace-nowrap opacity-0 transition-all group-hover:left-0 group-hover:opacity-100">
                        Back to log in
                    </p>
                </div>
